  ...
  return (
    <main className="pb-[140px] min-h-screen text-right font-sans relative bg-slate-900" dir="rtl">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Image src="/images/paint18.webp" alt="خلفية ديكورات فخمة مضيئة" fill priority className="object-cover" />
      </div>
      ...
      <div className="relative z-10">
        <header className="relative min-h-[550px] flex flex-col items-center justify-center text-center px-5 pt-20 pb-40 rounded-b-[50px] border-b border-white/10 bg-gradient-to-b from-black/80 via-black/40 to-transparent">
          <div className="bg-amber-500 text-white px-6 py-2 rounded-full text-sm font-black mb-6 shadow-md animate-pulse">خصم حصري 20% لجميع أحياء شمال الرياض ⚡</div>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-4 leading-tight drop-shadow-[0_5px_5px_rgba(0,0,0,0.8)]">مـعلم دهانات <br /><span className="text-brand-gold bg-gradient-to-r from-amber-400 to-yellow-500 bg-clip-text text-transparent">وديكورات الرياض</span></h1>
          <p className="max-w-3xl mx-auto text-lg text-white leading-relaxed font-black mb-8 drop-shadow-[0_3px_3px_rgba(0,0,0,0.8)]">نحول رؤيتك إلى واقع بأيدي خبراء. تشطيبات متكاملة، دهانات حديثة، وديكورات فخمة تضفي طابعاً ملكياً على مساحتك. نضمن لك أعلى معايير الجودة والتزاماً صارماً بموعد التسليم.</p>
          <div className="flex flex-col items-center">
            <p className="text-white text-base mb-2 font-bold drop-shadow-md">📞 احجز استشارتك المجانية الآن</p>
            <a href={`tel:${PHONE_NUMBER}`} className="text-5xl md:text-7xl font-black text-amber-400 drop-shadow-[0_4px_4px_rgba(0,0,0,0.9)] tracking-wider hover:scale-105 transition-transform">{PHONE_NUMBER}</a>
          </div>
        </header>
        ...
        <section className="px-5 max-w-6xl mx-auto -mt-24">
          <div className="rounded-[35px] overflow-hidden h-[350px] bg-black shadow-2xl border border-white/20">
            <video autoPlay muted loop playsInline className="w-full h-full object-cover"><source src="/videos/video1.webm" type="video/webm" /><source src="/videos/video1.mp4" type="video/mp4" /></video>
          </div>
        </section>

        <section className="px-5 max-w-6xl mx-auto mt-20">
          {SECTIONS.map((section) => (
            <article key={section.id} className="flex flex-wrap items-stretch bg-white/5 backdrop-blur-xl rounded-[40px] overflow-hidden mb-16 shadow-2xl border border-white/10 hover:border-amber-400/50 transition-all duration-300">
              <div className="flex-1 min-w-[300px] p-8 md:p-12 flex flex-col justify-center">
                <h2 className="text-3xl font-black text-white mb-4 drop-shadow-md"><span className="text-amber-400 ml-2">|</span>{section.title}</h2>
                <p className="text-white text-base leading-loose font-medium mb-6 drop-shadow-sm">{section.desc}</p>
                <ul className="space-y-3 mb-10">
                  {section.features.map((feature, i) => (<li key={i} className="flex items-start text-amber-50 font-semibold drop-shadow-sm"><span className="text-amber-400 ml-3 text-lg">✔</span>{feature}</li>))}
                </ul>
                <a href={section.link} className="inline-block bg-gradient-to-r from-amber-500 to-yellow-600 text-white px-8 py-4 rounded-full font-black shadow-lg hover:brightness-110 transition-all w-fit">الدخول لمعرض القسم ←</a>
              </div>
              <div className="flex-1 min-w-[300px] relative min-h-[400px] group overflow-hidden">
                <Image src={`/images/paint${section.imgNum}.webp`} alt={section.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-black/20 flex items-end justify-center p-8 opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-[2px]">
                  <div className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-6 py-3 rounded-2xl font-bold shadow-2xl">👆 اضغط لفتح {section.title}</div>
                </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl p-12 rounded-[40px] text-center shadow-2xl border border-white/10">
